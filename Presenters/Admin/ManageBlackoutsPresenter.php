...
class ManageBlackoutsPresenter extends ActionPresenter
{
	...
	public function UpdateBlackout()
	{
		...
		$st = $startDate . ' ' . date("H:i", strtotime($startTime));
		$et = $endDate . ' ' . date("H:i", strtotime($endTime));
		
		
		//$this->updatingBlockouts($id,$st,$et);
		...
		$result = $this->manageBlackoutsService->Update($id, $st, $et, $blackoutDate, $resourceIds, $title, ReservationConflictResolution::Create($conflictAction),
		    $repeatOptions, $scope);
