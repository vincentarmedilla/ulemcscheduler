...
<script>
	$( document ).ready(function() {
		$("select[name=userFilter]").change(function(){
		...
		<div class="panel-body">
			...
			<form id="filterForm" role="form">
				...
				<div class="form-group filter-user {$groupClass}">
					...
					<select id="userFilter" name="userFilter" class="form-control selectpicker" data-live-search="true">
				...
				<div class="form-group filter-resource {$groupClass}">
					...
					<select id="resourceId" name="resourceId" class="form-control" data-live-search="true">
						<option value="">All Resources</option>
						{foreach from=$resources key=myId item=i}
							<option label="{$i.name}" value="{$i.resource_id}">
								{$i.name}
							</option>
						{/foreach}
					</select>
                ...
                <div class="form-group filter-title {$groupClass}">
					...
					<input id="referenceNumber" type="hidden" class="form-control" value="{$ReservationDescription}"
						   placeholder="{translate key=Description}"/>
					<div style="display: none">
					<select id="statusId" class="form-control">
						<option value="">All Reservations</option>
						<option value="3">Pending Reservations</option>
					</select>
					</div>
	...
	<table class="table admin-panel" id="reservationTable">
		...
		{foreach from=$reservations item=reservation}
			...
			<tr class="{$rowCss} {if $IsDesktop}editable{/if}" data-seriesId="{$reservation->SeriesId}" data-refnum="{$reservation->ReferenceNumber}">
				...
				<td class="reservationTitle">{$reservation->Title|truncate:30:"...":true}</td>
				<td class="description">{$reservation->Description|truncate:30:"...":true}</td>
