function Reservation(opts) {
    ...
    var elements = {
        ...
        inviteeAutocompleteHuman: $('#inviteeAutocompleteHuman'),
        ...
        resourceGroupsDialogHuman: $('#dialogResourceGroupsHuman'),
        addResourcesConfirm: $('.btnConfirmAddResources'),
        addResourcesConfirmHuman: $('.btnConfirmAddResourcesHuman'),
    ...
    ajaxGet(urls, null, function (data) {
        $.each(data, function (i, unavailableResourceId) {
            $('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').prop('disabled', true);
            $('#group_user_list_human_engineer').selectpicker('refresh');
        });
        
        $.ajax({
            url: 'block_resource.php',
            type: 'POST',
            dataType: "json",
            data: { Bd:elements.beginDate.val(), Ed:elements.endDate.val(), Bt:elements.beginTime.val(), Et:elements.endTime.val()} ,
            cache: false,
            success: successed,
            error: function () {
                alert("error");
        ...
        function successed(responsed)
        {
        	
        	$.each(responsed, function (index, obj)
        	{
        		 $('#group_user_list_human_engineer option[resource-id-human-engineer="'+obj.resource_id+'"]').prop('disabled', true);
                 $('#group_user_list_human_engineer').selectpicker('refresh');
       ...
       // $('#group_user_list_human_engineer option[resource-id-human-engineer="1183"]').prop('disabled', true);
       // $('#group_user_list_human_engineer').selectpicker('refresh');
    });

    $("#BeginDate").bind("change paste keyup", function() {
        var urls1 = options.resourcesUrl
            .replace('[sd]', elements.beginDate.val())
            .replace('[ed]', elements.beginDate.val())
            .replace('[rn]', elements.referenceNumber.val())
            .replace('[st]', elements.beginTime.val())
            .replace('[et]', elements.endTime.val())
            .replace('[rn]', elements.referenceNumber.val());
        ...
        $("#group_user_list_human_engineer option:selected").attr('disabled','disabled').siblings().removeAttr('disabled');

        ajaxGet(urls1, null, function (data) {
            $.each(data, function (i, unavailableResourceId) {
                $('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').prop('disabled', true);
                $('#group_user_list_human_engineer').selectpicker('refresh');
            });
    ...
    Reservation.prototype.init = function (ownerId, startDateString, endDateString) {
        ...
        $('#dialogResourceGroupsHuman').on('show.bs.modal', function (e) {

            InitializeAdditionalResourcesHuman();
            return true;
        });
        ...
        $('.btnClearAddResources').click(function () {
            elements.resourceGroupsDialogHuman.modal('hide');
        });
        ...
        elements.addResourcesConfirmHuman.click(function () {
            AddResourcesHuman();
        });
    ...
    Reservation.prototype.addResourceGroups = function (resourceGroups) {
        elements.groupDiv.tree({
            ...
            onCreateLi: function (node, $li) {
                ...
                //alert(cname);
                if(cname == 'email') {
                    var label = $('<label id="demos"><input type="checkbox"/>'+ itemName +' '+ node.email+'</label>');
                } else if(cname == 'nomail') {
                    var label = $('<label id="demos"><input type="checkbox"/>'+ itemName +' '+ node.email+'</label>')
                }
    ...
    /*var AddResources = function () {
        ...
        var rs = $("input[name='nomail']:checked").length;
        var hm = $("input[name='email']:checked").length;
        var h = $("#human").val();
        /!*alert(rs);
        alert(hm);*!/
        /!*alert($('.additionalResourceCheckbox:checked').size());
        $("#inviteeAutocomplete").attr("disabled", "disabled");*!/
        //if(hm >= rs) {
        ...
        var allCheckboxesHuman = elements.resourceGroupsDialogHuman.find('.additionalResourceCheckbox:checked');
        ...
        $.each(allCheckboxesHuman, function (i, checkbox) {
            var checkedResourceId = $(checkbox).attr('resource-id');
            if (!_.includes(addedResources, checkedResourceId)) {
                checkboxes.push(checkbox);
                addedResources.push(checkedResourceId);
        ...
        if (checkboxes.length >= 1) {
            ...
            $.each(checkboxes, function (i, checkbox) {
                ...
                displayDiv.append('<div class="resourceName" id="'+checkedResourceId+'" style="background-color:' + color + '; color:' + textColor + ';">' + '<span class="resourceDetails" data-resourceId="' + checkedResourceId + '">' + checkedResourceName + '</span> ' + '<input class="resourceId" type="hidden" name="additionalResources[]" value="' + checkedResourceId + '"/>' + (requiresApproval ? ' <i class="fa fa-lock" data-tooltip="approval"></i> ' : '') + (requiresCheckin ? ' <i class="fa fa-sign-in" data-tooltip="checkin"></i> ' : '') + (!_.isEmpty(autoReleaseMinutes) ? ' <i class="fa fa-clock-o" data-tooltip="autorelease" data-autorelease="' + autoReleaseMinutes + '"></i> ' : '') + '</div>');
                //displayDiv.append('<div class="user"><a href="#" class="remove"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="154" user-details-bound="1">Gerardo Abrego (Gerardo.Abrego@ul.com)</a><input type="hidden" class="id" name="invitationList[]" value="154"></div>');
            });
        }
        ...
        /!* } else {
             if (confirm("You are booking more Resource that User. Do you wish to continue?")) {
                 var displayDiv = elements.additionalResources;
                 displayDiv.empty();
                 ...
                 $('#dialogResourceGroups').modal('hide');
             }
         }*!/
        /!* if (confirm("You are booking more Resource that User. Do you wish to continue?")) {
             var displayDiv = elements.additionalResources;
    ...
    var AddResources = function () {
        ...
        if (checkboxes.length >= 1) {
            ...
            $.each(checkboxes, function (i, checkbox) {
                ...
                $(".pr_"+checkedResourceId).prop( "checked", true );
                $(".hr_"+checkedResourceId).prop( "checked", true );
                displayDiv.append('<div class="resourceName" id="' + checkedResourceId + '" style="background-color:' + color + '; color:' + textColor + ';">' + '<span class="resourceDetails" data-resourceId="' + checkedResourceId + '">' + checkedResourceName + '</span> ' + '<input class="resourceId" type="hidden" name="additionalResources[]" value="' + checkedResourceId + '"/>' + (requiresApproval ? ' <i class="fa fa-lock" data-tooltip="approval"></i> ' : '') + (requiresCheckin ? ' <i class="fa fa-sign-in" data-tooltip="checkin"></i> ' : '') + (!_.isEmpty(autoReleaseMinutes) ? ' <i class="fa fa-clock-o" data-tooltip="autorelease" data-autorelease="' + autoReleaseMinutes + '"></i> ' : '') + '</div>');
            });
        } else {
        	 primaryResourceContainer.find('.resourceName').remove();
             displayDiv = primaryResourceContainer;
             $('#primaryResourceId').val('');
        }
    ...
    var AddResourcesHuman = function () {
        var displayDiv = elements.additionalResources;
        displayDiv.empty();
        ...
        var resourceIdHdn = primaryResourceContainer.find('.resourceId');
        var resourceId = resourceIdHdn.val();
        var allCheckboxes = elements.resourceGroupsDialogHuman.find('.additionalResourceCheckbox:checked');

        var checkboxes = [];
        var addedResources = [];
        $.each(allCheckboxes, function (i, checkbox) {
            var checkedResourceId = $(checkbox).attr('resource-id-human-checkbox');
            if (!_.includes(addedResources, checkedResourceId)) {
                checkboxes.push(checkbox);
                addedResources.push(checkedResourceId);
            }
        ...
        if (checkboxes.length >= 1) {
            $("#inviteeList").empty();
            resourceIdHdn.val($(checkboxes[0]).attr('resource-id-human-checkbox'));
            $.each(checkboxes, function (i, checkbox) {
                displayDiv = elements.additionalResources;
                var checkedResourceId = $(checkbox).attr('resource-id-human-checkbox');
                var checkedResourceName = $(checkbox).parent().text();
                var checkedResourceNValue = $(checkbox).val();
                var res = checkedResourceNValue.split("=>");
                var name = res[0];
                var userId = res[1];
                ...
                if(checkedResourceNValue != '0') {
                    var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' +checkedResourceId + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
            ...
            $("#dialogResourceGroupsHuman").modal('hide');
        }
    ...
    /*var AddResourcesHuman = function () {
        ...
        if(hm >= rs || rs <= '20' || rs >= rs) {

            var displayDiv = elements.additionalResources;
            displayDiv.empty();

            var primaryResourceContainer = $('#inviteeList');
            var resourceIdHdn = primaryResourceContainer.find('.resourceId');
            var resourceId = resourceIdHdn.val();
    ...
    var InitializeAdditionalResources = function () {
        ...
        /*var allSelect = dialog.find('[resource-id-pull]');
        allSelect.prop('disabled', false);
        allSelect.parent().removeClass('unavailableResource');*/
        $("#physical_list option:selected").attr('disabled','disabled').siblings().removeAttr('disabled');

        dialog.find('#checking-availability').removeClass('no-show');

        ajaxGet(url, null, function (data) {

            // disable the pulldown in reservation
            $.each(data, function (i, unavailableResourceId) {
                $("#physical_list option[resource-id-pull='"+ unavailableResourceId + "']").attr('disabled', true);
                //$('#physical_list').select2();
            });

            //disable the checkbox in reservation
            $.each(data, function (i, unavailableResourceId) {
    ...
    var InitializeAdditionalResourcesHuman = function () {
        ...
        var dialog = elements.resourceGroupsDialogHuman;
        var allCheckboxes = dialog.find('[resource-id]');
        allCheckboxes.prop('disabled', false);
        allCheckboxes.parent().removeClass('unavailableResource');

        var allSelect = dialog.find('[resource-id-human]');
        allSelect.prop('disabled', false);
        allSelect.parent().removeClass('unavailableResource');

        dialog.find('#checking-availability-human').removeClass('no-show');

        ajaxGet(url, null, function (data) {
            $.each(data, function (i, unavailableResourceId) {
                $('#group_user_list_human option[resource-id-human="'+unavailableResourceId+'"]').prop('disabled', true);
                $('#group_user_list_human').selectpicker('render');
            });
            ...
            dialog.find('#checking-availability-human').addClass('no-show');
        });
    ...
    var ShowReservationAjaxResponse = function () {
        var returnUrl = options.emailUrl;
        var res = returnUrl.split("=");

        if(res[1] == "") {
            //url = options.returnUrl.replace(/&amp;/g, '&');
    ...
    var WireUpActions = function () {
        ...
        $('.create').click(function () {
            ...
            var human = $('.hr:checked').size();
            var t = $('.pr:checked').size();
            ...
            if(t <= '0' || human <= '0') {
                ...
                if(t == '1' || human == '1') {
                    ...
                    if (confirm("You are booking 1 Human Resource for more than 1 Physical Resource. Do you wish to continue?")) {
                        $('form').attr("action", options.createUrl);
                    } else {
                        //location.reload();
                        request.abort();
...
//        });
    ...
    function InitializeParticipationElements() {
        ...
        elements.hrList.delegate('.remove', 'click', function (e) {
            e.preventDefault();
            var item = $(this).closest('.user');
            var id = item.find('.id').val();
            item.remove();
        ...
        elements.inviteeAutocompleteHuman.userAutoComplete(options.userAutocompleteUrl, function (ui) {
            $("#inviteeAutocomplete").attr("disabled", "disabled");
    ...
    /* participation.addInvitee = function (name, userId) {
        ...
        if($('input[name=reservationAction]').val() != 'update')  {
             $.ajax({
                 url: 'check_resource_human.php',
                 type: 'POST',
                 dataType: "json",
        ...
        function success(response) {
            if(response!== false) {
                var primaryResourceContainer1 = $('#additionalResources');
                displayDiv2 = primaryResourceContainer1;
                displayDiv2.append('<div class="resourceName" id="' + response["resource_id"] + '" style="background-color:; color:;"><span class="resourceDetails" data-resourceid="' + response["resource_id"] + '" resource-details-bound="1">\n' +
                    '\n' + response["name"] + '</span> <input class="resourceId" type="hidden" name="additionalResources[]" value="' + response["resource_id"] + '"></div>');
                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + response["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
                elements.inviteeList.append(item);
                $('.bindableUser').bindUserDetails();
                participation.addedUsers.push(userId);
                updateInviteeCount();
         ...
         if($('input[name=reservationAction]').val() == 'update' || $('input[name=reservationAction]').val() == 'create') {
             var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + userId + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
    ...
    $('#project_lead').on('change', function() {
       ...
       // if ($.inArray(userId, participation.addedUsers) >= 0) {
        ...
        function success(response) {
            //$("#"+response["resource_id"]).prop( "checked", true );
    ...
    $('#group_user_list').on('change', function() {
        var str = $(this).find(":selected").val();
        var res = str.split("=");
        var name = res[0];
        var userId = res[1];
    ...
    $('#group_user_list_human').on('change', function() {
        ...
        function success(response) {
            ...
            $("#"+response["resource_id"]).prop( "checked", true );
            $("input:checkbox[name=hr_"+response['resource_id']+"]").attr("checked",true);
    ...
    $('#physical_list').on('change', function() {

        var rid = $(this).find(":selected").val();
    ...
    $('#group_user_list_human_engineer').on('change', function() {

    	var str = $(this).find(":selected").val();
        var res = str.split("=");
        var name = res[0];
        var userId = res[1];
        var resourceId = res[2];
        
        
        $('#group_user_list_human_engineer option[resource-id-human-engineer="'+res[2]+'"]').prop('disabled', true);
        $('#group_user_list_human_engineer').selectpicker('refresh');
        //if($("#primaryResourceId").val().length == '0') {
           // $('#primaryResourceId').val(resourceId);
        //}
    ...
    if($('input[name=reservationAction]').val() == 'create') {
        $.ajax({
            url: 'get_hr.php',
            type: 'POST',
            dataType: "json",
            data: jQuery.param({ rid: $.urlParam('rid')}) ,
            cache: false,
            success: successed,
            error: function () {
                alert("error");
        ...
        function successed(responsed)
        {
            if(responsed["user_id"] != undefined) {
                //var item = '<div class="user">' + '<a href="#" class="remove"></a> <a href="#" class="bindableUser" data-userid="' + responsed["user_id"] + '">' + responsed["fname"] +  ' ' + responsed["lname"] + '('+responsed["email"]+')</a><input type="hidden" class="id primary_user_id" name="invitationList[]" value="' + responsed["user_id"] + '" />' + '</div>';
                

                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + responsed["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + responsed["user_id"] + '">' + responsed["fname"] +  ' ' + responsed["lname"] + '('+responsed["email"]+')</a><input type="hidden" class="id primary_user_id" name="invitationList[]" value="' + responsed["user_id"] + '" />' + '</div>';

            }
            //alert(responsed["resource_id"]);

            elements.inviteeList.append(item);
            $('.bindableUser').bindUserDetails();
            participation.addedUsers.push(responsed["user_id"]);
            updateInviteeCount();
        }

    }

    //add test engineers in adding reservation page
    participation.addInvitee = function (name, userId) {
        ...
        if($('input[name=reservationAction]').val() == 'update') {
            ...
            function successed(responsed)
            {
            	$('#group_user_list_human_engineer option[resource-id-human-engineer="'+responsed["resource_id"]+'"]').prop('disabled', true);
                //alert(responsed["resource_id"]);
                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + responsed["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
...
function remove_user(id) {
    /*if(id == $('#primaryResourceId').val()) {

    } else {
        $('#'+id).remove();
        $("#"+id).prop( "checked", false );
        //$("input:checkbox[name=hr_"+id+"]").attr("checked",false);
        //working with add, edit and duplicate
        $(".btnConfirmAddResources").click();
    }*/

    $('#'+id).remove();
    $("#"+id).prop( "checked", false );
    $('#group_user_list_human_engineer option[resource-id-human-engineer="'+id+'"]').prop('disabled', false);
    $('#group_user_list_human_engineer').selectpicker('refresh');
    $('#primaryResourceId').val("");
    //$("input:checkbox[name=hr_"+id+"]").attr("checked",false);
    //working with add, edit and duplicate
...
$(".additionalResourceCheckbox").change(function(event){
	//alert($('.pr:checked').length);
	//alert($('.hr:checked').length);
	if($('.pr:checked').length == '0') {
		$(".btnConfirmAddResources").attr("disabled", false);
	} else {
		$(".btnConfirmAddResources").attr("disabled", false);
	}
});
...
$(document).ready(function() {
    $("#btnAddResources").click(function() {
    	//reset the checkbox
    	$('.pr').prop('checked', false);
    	
    	//get the additional resources ids
    	var IDs = [];
    	$("#additionalResources").find("div").each(function(){ IDs.push($(this).attr("id")); });
    	
    	var resourceId = $('#primaryResourceId').val();
    	$('.pr_'+resourceId).prop("checked", true );
    	
    	//check the checkbox based on the ids
    	$("#dialogResourceGroups").find('[value=' + IDs.join('], [value=') + ']').prop("checked", true);
    });
});
