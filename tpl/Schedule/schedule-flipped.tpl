                    ...
                    <td class="resourcename" resourceId="{$resource->Id}" {if $resource->HasColor()}style="background-color:{$resource->GetColor()} !important"{/if}>
						{if $resource->CanAccess}
                            <a href="{$href}" resourceId="{$resource->Id}"
                               class="resourceNameSelector" {if $resource->HasColor()}style="color:{$resource->GetTextColor()} !important"{/if}>{assign var="rname" value=" "|explode:$resource->Name}{$rname[1]} {$rname[2]} {$rname[3]} {$rname[4]} {$rname[5]} {$rname[6]} {$rname[7]}</a>
						{else}
							<span resourceId="{$resource->Id}" resourceId="{$resource->Id}" class="resourceNameSelector" {if $resource->HasColor()}style="color:{$resource->GetTextColor()} !important"{/if}>{assign var="rname" value=" "|explode:$resource->Name}{$rname[1]} {$rname[2]} {$rname[3]} {$rname[4]} {$rname[5]} {$rname[6]} {$rname[7]}</span>
						{/if}
                    </td>
				{/foreach}
            </tr>
