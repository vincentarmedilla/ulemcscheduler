		...
		<div>
			...
			<select id="calendarFilter">
				{foreach from=$filters->GetFilters() item=filter}
					...
					{if {$filter->Name()} neq 'All Reservations'}
					<optgroup label="{$filter->Name()}">
					<option value="s{$filter->Id()}" class="schedule" {if $filter->Id() == $MySchdId}selected="selected"{/if}><div style="color:red;">{$filter->Name()}</div></option>
					{/if}
