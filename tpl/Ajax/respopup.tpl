...
<!--    {formatdate date=$startDate key=res_popup} - {formatdate date=$endDate key=$key} <br> -->
    ...
    <strong>Title:</strong> {if $title neq ''}{$title|truncate:30:"...":true}{else}{translate key=NoTitleLabel}{/if}<br>
    {/if}
    
    {if in_array('description', $userFields)}
    <strong>Description:</strong> {if $summary neq ''}{$summary|truncate:30:"..."|nl2br}{else}{translate key=NoDescriptionLabel}{/if} <br>
    {/if}
...
{/foreach}<br>*}
    ...
    <div><strong>Client Name:</strong> {$client_name}</div><br>
    {/if}
    ...
    <strong>Client on site:</strong> {if $cos eq '1'} Yes {else} No {/if}<br>
    {/if}
    
     {if in_array('sampleonsite', $userFields)}
    <strong>Sample on site:</strong> {if $sos eq '1'} Yes {else} No {/if}
    {/if}
