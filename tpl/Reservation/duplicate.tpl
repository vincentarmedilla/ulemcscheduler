...
<script>
    $( document ).ready(function() {
        $('#group_user_list').selectize({
            create: true,
            sortField: 'text'
        });

        $('#project_lead').selectize({
            create: true,
            sortField: 'text'
        });

        $('#group_user_list_human').selectize({
            create: true,
            sortField: 'text'
        });
                    ...
                    <div class="col-xs-12 reservationDescription">
                        <div class="form-group has-feedback">
                            ...
                            <textarea id="description" name="{FormKeys::DESCRIPTION}"
                                      class="form-control" data-validation="required" data-validation-length="min4" data-validation-error-msg="Description is Required"
                ...
                <div class="col-xs-12 col-sm-6 detailsBorder" id="reservationDetails">
                    <div class="col-xs-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="client" name="client" data-validation="required" data-validation-length="" required data-validation-error-msg="Client is Required" value="{$client_name}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="resourcesModalLabel">Physical Resource{$resource_id|@print_r}</h4>
                               ...
                               {* {if {$row->email} eq ''}*}
                                ...
                                <span class="jqtree-title jqtree_common">
                                    <label id="demos">
                                            ...
                                            <input {if $row->resource_id|in_array:$resource_id}{*{if {$rsr} eq {$row->resource_id}}*} checked {/if} type="checkbox" resource-id="{$row->resource_id}" id="{$row->resource_id}" name="nomail" group-id="0" reservation-color="" reservation-text-color="" requires-approval="0" requires-checkin="0" class="additionalResourceCheckbox">
                                        {$row->resource_name}
                    ...
                    <div class="participationText">
                        ...
                        {else}
                            ...
                            <select id="group_user_list_human" class="form-control">
                                ...
                                {foreach from=$result->children item=row}
                                    {if {$row->email} neq ''}
                                        <option value="{$row->firstname} {$row->lastname}({$row->email}) = {$row->userid}">
