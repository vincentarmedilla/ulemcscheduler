function GroupManagement(opts) {
	...
	GroupManagement.prototype.init = function() {
		...
		$('#userSearchGroups').on('change', function() {
			var gid = $('#gid').val();
			var uid = this.value;

			$.ajax({
				url: 'add_users_group.php',
				type: 'POST',
				data: jQuery.param({ gid: gid, uid:uid}) ,
				cache: false,
			...
			if (data.Users != null)
			{
				$.map(data.Users, function(item) {
					...
					/*$.ajax({
						...
						type: 'POST',
						dataType: "json",
						data: jQuery.param({ uid: item.Id, gids:groupId}) ,
						cache: false,
						success: function (response) {
