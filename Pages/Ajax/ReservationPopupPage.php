...
class ReservationPopupPage extends Page implements IReservationPopupPage
{
	...
	public function PageLoad()
	{
		...
		$rid = $this->GetReservationId();

        $obj = new bookingClass;
        
        $result = $obj->getReservationInstanceSeries($rid);
        $userField = $obj->getSchd($userSession->UserId);
        
        $te = array();
        $re = $obj->getTestEngineers($result['reservation_instance_id']);
        ...
        $this->Set('resource',$obj->getResourcesInfo($_GET['id']));
        $this->Set('userFields',json_decode($userField['fields']));
        $this->Set('status',$result['status']);
        $this->Set('client_name',$result['client_name']);
        $this->Set('project',$result['project']);
        $this->Set('request_no',$result['request_no']);
        $this->Set('cos',$result['client_on_site']);
        $this->Set('sos',$result['sample_on_site']);
        $this->Set('eng',$te);
        $this->Set('engineers',$obj->getTestEngineers($result['reservation_instance_id']));
