...
class ReportCommandBuilder
{
	...
	const RESERVATION_LIST_FRAGMENT = 'rs.date_created as date_created, rs.request_no as rn, rs.last_modified as last_modified, rs.repeat_type, SUBSTRING(rs.description,1,20) as description,
    ...
    const PROJECT_FRAGMENT = 'AND rs.project = @project';

    const PROJECT_CLIENT= 'AND rs.client_name like "%appl%"';
    ...
    public function WithProjectNo($projectNo)
    {
    ...
    public function WithProjectLead($projectLead)
    {
	...
	private function GetWhereAnd()
	{
        ...
        {
            return 'AND rs.project like "%'.$this->projectNo.'%"';
            //$and->Append(self::PROJECT_FRAGMENT);
            //$this->AddParameter(new Parameter(ParameterNames::PROJECT, $this->projectNo));
        }

        if (!empty($this->clientName))
        {
            return 'AND rs.client_name like "%'.$this->clientName.'%"';
           // $and->Append(self::PROJECT_CLIENT);
            //$this->AddParameter(new Parameter(ParameterNames::CLIENT_NAME, $this->clientName));
        }
