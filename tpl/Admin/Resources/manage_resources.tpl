...
<script>
	$( document ).ready(function() {
		...
		$('#list-resources-panel').find('input:hidden').each(function(){
			var rid = $(this).val();
			$.ajax({
				url: 'get_human_resource.php',
				type: 'POST',
				data: jQuery.param({ rid: rid}) ,
				cache: false,
				success: function (response) {
					$("#hr"+rid).html(response);
					//alert(response);
				},
				error: function () {
					alert("error");
		...
		$('select').on('change',function(){
			...
			$.ajax({
				url: 'update_human_resource.php',
				type: 'POST',
				data: jQuery.param({ uid: uid, rid:rid}) ,
				cache: false,
				success: function (response) {
					//alert(response);
				},
		...
		/*$('#human_resources_list').selectize({
			sortField: 'text',
		});*/

		$('#human_users').hide();
		$("#human_resource").change(function() {
			if ($(this).val() == "Y") {
				$('#human_users').show();
			} else {
				$('#human_users').hide();
...
//        live: 'disabled',
			...
			fields: {
				resourceName: {
					message: 'Resource Name Already Taken',
					validators: {
						notEmpty: {
							message: 'The resource name is required and can\'t be empty'
						},
						remote: {
							type: 'POST',
							url: 'check.php',
				...
				user_engineer: {
					...
					validators: {
						...
						remote: {
							type: 'POST',
							url: 'check_user.php',
							message: 'User already assigned to a resource',
							delay: 1000
						}
					}
				}
	...
	$(function () {
		$(".DialogFilter").click(function () {
			$("#groupDialogFilter").modal("show");

		});

		$('#add-resource-dialog').on('hidden.bs.modal', function () {
			$(this).find('form').trigger('reset');
			$("#user_engineer").val('0').selectpicker('refresh');
			$('#addResourceForm').bootstrapValidator('resetForm', true);
		})
	});
			...
			<div class="panel-body">
				...
				<div class="form-group {$groupClass}">
					...
					<select id="filterScheduleId" name="scheduleId" class="form-control selectpicker" data-live-search="true">
						...
						{foreach from=$schedules key=myId item=i}
							<option label="{$i.name}" value="{$i.schedule_id}">
								{$i.name}
							</option>
						{/foreach}
					</select>
				...
				<div class="form-group {$groupClass}">
					...
					<select id="filterResourceType" class="form-control" {formname key=RESOURCE_TYPE_ID}>
                                            ...
                                            {foreach from=$resourceTyped key=myId item=i name=foo}
				...
				<div class="resourceDetails" data-resourceId="{$id}">
					<div class="col-xs-12 col-sm-5">
						...
						<div class="col-sm-9 col-xs-6">
							...
							<div style="float: left"><div id="" style="float: right"></div>
								Human Resource:<a href="#"></a><div id="hr{$id}" style="float: right"></div>
								<select class="form-control" id="human_resources_list" name="human_resources_list">
									<option> Select User</option>
									{foreach $users_engineer as $ue}
										<option value="{$ue.user_id} => {$id}">{$ue.fname} {$ue.lname}</option>
									{/foreach}
								</select>
...
<!-- 								   target="_blank">{translate key=PrintQRCode} <i class="fa fa-qrcode"></i></a> -->
					...
					<div class="col-xs-12 col-sm-7">
						...
						<div class="col-sm-6 col-xs-12">
							...
							<a href="#" data-id="{$resource->GetResourceId()}" class="update changeUserPermission">{translate key=Users}</a> |
							<a href="#" data-id="{$resource->GetResourceId()}" class="update changeGroupPermissions">{translate key=Groups}</a>
							{*<a href="#" class="DialogFilter"  data-id="{$resource->GetResourceId()}" >Groupse</a>*}
						</div>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="form-group">
							...
							{else}
								<select id="scheduleId" name="scheduleId" class="form-control" data-live-search="true">
									{foreach from=$schedules key=myId item=i}
										<option label="{$i.name}" value="{$i.schedule_id}">
											{$i.name}
										</option>
					    ...
					    <div class="form-group">
							<label for="scheduleId">Human Resource</label>
							<select class="form-control human_resource" id="human_resource" name="human_resource">
						...
						<div class="form-group" id="human_users">

							<label for="scheduleId">Users</label>
							{if $CanViewAdmin}
							<select class="form-control selectpicker" id="user_engineer" name="user_engineer" data-live-search="true">
								<option> Select User</option>
								{foreach $all_users as $ue}
									<option value="{$ue.user_id}">{$ue.fname} {$ue.lname} </option>
								{/foreach}
							</select>
							{else}
								<select class="form-control selectpicker" id="user_engineer" name="user_engineer" data-live-search="true">
									<option> Select User</option>
									{foreach from=$all_users key=myId item=ue name=foo}
									<option value="{$ue.user_id}">{$ue.fname} {$ue.lname} ==  {$smarty.foreach.foo.iteration}</option>
								{/foreach}
								</select>
							{/if}
						</div>
						<div class="form-group">
							...
							<select class="form-control" {formname key=AUTO_ASSIGN} id="permissions">
								...
								<option value="0" selected>{translate key="ResourcePermissionNotAutoGranted"}</option>
							</select>
						</div>
						<div class="form-group">
							...
							<select class="form-control" {formname key=RESOURCE_ADMIN_GROUP_ID}
								...
								{foreach from=$resourceAdmin key=myId item=i}
...
<!-- 								{/foreach} -->
					...
					<div class="modal-footer">
						{cancel_button}
						{*<input type="submit" class="btn btn-success save" id="add_resource">*}
						<button type="submit" class="btn btn-success save" id="add_resource"><span class="glyphicon glyphicon-ok-circle"></span> Add</button>
						{indicator}
					</div>
				</div>
			</div>
				...
				<div class="form-group">
					<label>{translate key=Status}
						<select {formname key=RESOURCE_STATUS_ID} class="statusId form-control">
							<option value="{ResourceStatus::AVAILABLE}">{translate key=Available}</option>
							{*<option value="{ResourceStatus::UNAVAILABLE}">{translate key=Unavailable}</option>*}
							<option value="{ResourceStatus::UNAVAILABLE}">{translate key=Hidden}</option>
						</select>
					</label>
				</div>
...
<!-- 									<option value="{ResourceStatus::UNAVAILABLE}">{translate key=Unavailable}</option> -->
			...
			<div class="modal-content">
				...
				<div class="modal-body scrollable-modal-content">
					<div class="form-group">
						...
						<select id="search_code" class="js-example-basic-single" style="width:551px;">
							<option>Select User</option>
							{foreach $all_users as $ue}
								<option value="{$ue.user_id}">{$ue.fname} {$ue.lname}</option>
							{/foreach}
	...
	<div id="groupDialogFilter" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="browseGroupsDialogLabel"
		 aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
				...
				<div class="modal-body scrollable-modal-content">
					...
					{*<a href="#" id="browseGroups">{translate key=AllGroups}</a>*}
						<select id="group_id"  class="groups-permissions1">
							<option value="" selected>Select Group</option>
							{foreach from=$groups key=myId item=i}
								<option value="{$i.group_id}">
									{$i.name}
								</option>
							{/foreach}
					...
					<div id="resourceGroupListFilter"></div>
				</div>
			</div>
		</div>
	</div>
...
<!-- 						<input type="text" id="groupSearch" class="form-control" size="60"/> -->
						<select id="group_ids" class="groups-permissions" style="width:551px;">
							<option></option>
							{foreach from=$groups key=myId item=i}
								<option value="{$i.group_id}">
									{$i.name}
					...
					<input type="hidden" name="resourceIdGroup" id="resourceIdGroup" value=""/>
	...
	<script type="text/javascript">
		...
		$(document).on("click", ".changeUserPermission", function () {
			var resourceid = $(this).data('id');
			$(".modal-body #resource_id").val( resourceid );
			$('#userDialog').modal('show');
			
		});

		$(document).on("click", ".changeGroupPermissions", function () {
			var resourceid = $(this).data('id');
			$(".modal-body #resourceIdGroup").val( resourceid );
		...
		$(document).ready(function() {
			$('.js-example-basic-single').select2({
				...
				dropdownParent: $("#userDialog"),
				placeholder: "Select an option"
			}).on("select2:select", function (e) {

				var uid = $("#search_code").val();
				var rid = $("#resource_id").val();;

				$.ajax({
					url: "add_user_resource_permission.php",
					type: "POST",
					data: {
						uid: uid,
						rid:rid
					...
					success: function(dataResult){
						$("#resourceUserList").load('manage_admin_resources.php?dr=users&rid='+rid);
					}
				});
			});
		});
		...
		$(document).ready(function() {
			$('.groups-permissions').select2({

				dropdownParent: $("#groupDialog"),
				placeholder: "Select an option"
			}).on("select2:select", function (e) {

				var gid = $("#group_ids").val();
				var rid = $("#resourceIdGroup").val();
				

				$.ajax({
					url: "add_group_permission.php",
					type: "POST",
					data: {
						gid: gid,
						rid:rid
					...
					cache: false,
					success: function(dataResult){
						$("#resourceGroupList").load('manage_admin_resources.php?dr=groups&rid='+rid);
					}
				});
			});
		});
		...
		function setUpEditables() {
			...
			$('.scheduleName').editable({
				url: updateUrl + '{ManageResourcesActions::ActionChangeSchedule}', source: [
					{foreach from=$schedules item=scheduleName key=scheduleId}
					{
						value:{$scheduleName.schedule_id}, text: '{$scheduleName.name|unescape:'javascript'}'
					},
					{/foreach}
				]
			});
			...
			$('.resourceTypeName').editable({
				...
				source: [{
					...
					{foreach from=$resourceType item=resourceTypes key=id}
					{
                                            {if $resourceTypes.resource_type_name eq ''}
    
                                           {else}
                                               value:{$resourceTypes.resource_type_id}, text: '{$resourceTypes.resource_type_name|escape:'javascript'}'
                                           {/if}
					},
					{/foreach}
				]
			...
			$('.sortOrderValue').editable({
				url: updateUrl + '{ManageResourcesActions::ActionChangeSort}', emptytext: '0', min: 0, max: 999999
			});
			...
			$('.resourceAdminValue').editable({
				url: updateUrl + '{ManageResourcesActions::ActionChangeAdmin}', emptytext: '{{translate key=None}|escape:'javascript'}', source: [{
					...
					{foreach from=$resourceAdmin  item=group key=scheduleId}
					{
						value:{$group.group_id}, text: '{$group.name|escape:'javascript'}'
					},
					{/foreach}
				]
			});
