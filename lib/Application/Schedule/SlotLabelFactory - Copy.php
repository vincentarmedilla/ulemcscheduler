...
class SlotLabelFactory
{
    ...
    public function Format(ReservationItemView $reservation, $format = null)
    {
        ...
        if($client_name['client_on_site'] == '1') {
            $label = str_replace('{name}', "<span style=color:red;font-weight:bold>" . $client_name['client_name'] . ' | ' . rtrim($test_engineers, ", \t\n") . "</span>", $label);
            $label = str_replace('{title}', "<span style=color:red;>" . substr($reservation->Title,0, 14) . " |</span>", $label);
            $label = str_replace('{description}', "<span style=color:red;>".$reservation->Description. " |</span>", $label);
            $label = str_replace('{resourcename}', "<span style=color:red;>".implode(', ', $reservation->ResourceNames). " |</span>", $label);
            $label = str_replace('{startdate}', "<span style=color:red;>".$reservation->StartDate->ToTimezone($timezone)->Format($dateFormat). " |</span>", $label);
            $label = str_replace('{enddate}', "<span style=color:red;>".$reservation->EndDate->ToTimezone($timezone)->Format($dateFormat). " |</span>", $label);
        } else {
            $label = str_replace('{name}',  $client_name['client_name'] . ' | ' . rtrim($test_engineers, " \t\n"), $label);
            $label = str_replace('{title}', substr($reservation->Title,0, 14). ' |', $label);
