function ResourceManagement(opts) {
	...
	var elements = {
		...
		resourceGroupListFilter: $('#resourceGroupListFilter'),
	...
	$( "#groups_select" ).change(function() {
		var resourceId = $('#groups_select').val()
		$.get('manage_admin_resources.php?action=changeGroupPermission', {rid: resourceId}, function (data) {
			elements.resourceGroupListFilter.html(data);
			$('.group-permission-spinner').addClass('no-show');
		});
		$.get(opts.permissionsUrl + '?dr=groups', {rid: resourceId}, function (data) {
			elements.resourceGroupListFilter.html(data);
