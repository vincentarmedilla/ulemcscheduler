...
abstract class ReservationEmailMessage extends EmailMessage
{
	...
	protected function PopulateTemplate()
	{
        ...
        $this->Set('ClientName',$_POST['client']);
        $this->Set('OrderNo',$_POST['projects']);
        $this->Set('RequestNo',$_POST['request_no']);
        if($_POST['client_on_sites'] == 1) {
            $this->Set('ClientOnSite','Yes');
        } else {
            $this->Set('ClientOnSite','');
        }
        
        if($_POST['sample_on_site'] == 1) {
            $this->Set('SampleOnSite','Yes');
        } else {
            $this->Set('SampleOnSite','');
        }
	}
