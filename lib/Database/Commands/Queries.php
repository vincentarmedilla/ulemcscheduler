...
class Queries
{
    ...
    const ADD_RESERVATION_SERIES =
        'INSERT INTO
        reservation_series (date_created, title, description, allow_participation, allow_anon_participation, repeat_type, repeat_options, type_id, status_id, owner_id, terms_date_accepted, last_action_by, client_name, project, client_on_site, sample_on_site,request_no,status)
		VALUES (@dateCreated, @title, @description, @allow_participation, false, @repeatType, @repeatOptions, @typeid, @statusid, @userid, @terms_date_accepted, @last_action_by, @client, @project, @cos, @sos, @rn, @status)';
    ...
    /*const UPDATE_USER_GROUP = 'UPDATE user_groups
		    SET status = 0 WHERE user_id = @userid AND group_id = @groupid';*/
   ...
   const DELETE_USER_GROUP = 'DELETE FROM user_groups WHERE user_id = @userid AND group_id = @groupid';
    ...
    /*const GET_ALL_GROUP_USERS =
		...
		WHERE u.user_id IN (
		  SELECT DISTINCT (ug.user_id) FROM user_groups ug
		  INNER JOIN groups g ON g.group_id = ug.group_id
		  WHERE g.group_id IN (@groupid) OR ug.status != 0
		  )
		AND (0 = @user_statusid OR u.status_id = @user_statusid)
		ORDER BY u.lname, u.fname';*/
    ...
    const GET_ALL_RESOURCES =
        'SELECT r.*,u.user_id,u.fname,u.lname, s.admin_group_id as s_admin_group_id,
		(SELECT GROUP_CONCAT(CONCAT(cav.custom_attribute_id, \'=\', cav.attribute_value) SEPARATOR "!sep!")
						FROM custom_attribute_values cav WHERE cav.entity_id = r.resource_id AND cav.attribute_category = 4) as attribute_list,
		(SELECT GROUP_CONCAT(rga.resource_group_id SEPARATOR "!sep!") FROM resource_group_assignment rga WHERE rga.resource_id = r.resource_id) AS group_list,
		(SELECT GROUP_CONCAT(ri.image_name SEPARATOR "!sep!") FROM resource_images ri WHERE ri.resource_id = r.resource_id) AS image_list
    ...
    const REMOVE_RESERVATION_USER =
        'DELETE FROM reservation_users WHERE reservation_instance_id = @reservationid AND user_id = @userid AND reservation_user_level = 3';
    
    const REMOVE_RESERVATION_USER_PARTICIPANT =
    'DELETE FROM reservation_users WHERE reservation_instance_id = @reservationid AND user_id = @userid AND reservation_user_level = 2';
    ...
    const ADD_RESOURCE_WITH_E =
        'INSERT INTO
			resources (name, location, contact_info, description, notes, status_id, min_duration, min_increment,
					   max_duration, unit_cost, autoassign, requires_approval, allow_multiday_reservations,
					   max_participants, min_notice_time_add, max_notice_time, schedule_id, admin_group_id, date_created, userid)
		VALUES
			(@resource_name, @location, @contact_info, @description, @resource_notes, @status_id, @min_duration, @min_increment,
			 @max_duration, @unit_cost, @autoassign, @requires_approval, @allow_multiday_reservations,
		     @max_participants, @min_notice_time_add, @max_notice_time, @scheduleid, @admin_group_id, @dateCreated, @userid)';
...
class QueryBuilder
{
    ...
    {
        //echo self::Build(self::$SELECT_LIST_FRAGMENT, null, 'AND ru.reservation_user_level = @levelid');
