...
class ReservationItemView implements IReservedItemView
{
    ...
    )
    {
        ...
        $this->Client_name =  $get_reservation['client_name'];
        $this->Request_No = $get_reservation['request_no'];
        
        $te = array();
        $re = $obj->getProjectLead($get_reservation['reservation_instance_id']);
        
        foreach ($re as $result){
            $te[] = $result['fname'].' '.$result['lname'];
