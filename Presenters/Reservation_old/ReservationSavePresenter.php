...
class ReservationSavePresenter implements IReservationSavePresenter
{
	...
	public function BuildReservation()
	{
		...
		$reservationSeries->ChangeHr($hrIds);
	...
	public function HandleReservation($reservationSeries)
	{
		...
		if ($successfullySaved)
		{
            ...
            $getId = $obj->getIdInstance($reservationSeries->CurrentInstance()->ReferenceNumber());

            $hrIds = $this->page->GetHr();
            foreach ($hrIds as $userid){

               $obj->insertReservationUsers($userid,$getId['reservation_instance_id']);
            }
		}
	}
