...
class SlotLabelFactory
{
    ...
    public function Format(ReservationItemView $reservation, $format = null)
    {
        ...
        $test_engineers = "";
        foreach ($reservation->InviteeNames as $value) {
            $test_engineers .= "$value, &nbsp";
        }
        //print_r($reservation);exit();
        
        if($reservation->client_on_site == '1') {
            $cos = $reservation->ReservationId;
        } else {
            $cos = $reservation->ReservationId;
        }
        ...
        //get client name
        $client_name = $obj->getClientName($reservation->ReservationId);
        
        $label = $format;
        //$label = str_replace('{name}', $name, $label);
        
        if($reservation->client_on_site == '1') {
            $label = str_replace('{name}', "<span style=color:black;font-weight:bold>" . $client_name['client_name'] . ' | ' . rtrim($test_engineers, ", \t\n") . "</span>", $label);
            $label = str_replace('{title}', "<span style=color:black;font-weight:bold>" . substr($reservation->Title,0, 25) . " |</span>", $label);
            $label = str_replace('{status}',  $client_name['status'] , $label);
            $label = str_replace('{description}', "<span style=color:black;font-weight:bold>".$reservation->Description. " |</span>", $label);
            $label = str_replace('{resourcename}', "<span style=color:black;font-weight:bold>".implode(', ', $reservation->ResourceNames). " |</span>", $label);
            $label = str_replace('{startdate}', "<span style=color:black;font-weight:bold>".$reservation->StartDate->ToTimezone($timezone)->Format($dateFormat). " |</span>", $label);
            $label = str_replace('{enddate}', "<span style=color:black;font-weight:bold>".$reservation->EndDate->ToTimezone($timezone)->Format($dateFormat). " |</span>", $label);
        } else {
            $label = str_replace('{name}',  $client_name['client_name'] . ' | ' . rtrim($test_engineers, " \t\n"), $label);
            $label = str_replace('{title}', substr($reservation->Title,0, 25). ' |', $label);
            $label = str_replace('{status}', $client_name['status'], $label);
        ...
        //$label = str_replace('{title}', substr($reservation->Title,0,20).'....', $label);
        //$label = str_replace('{description}', $reservation->Description, $label);
