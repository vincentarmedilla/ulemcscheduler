function Reservation(opts) {
            ...
            if (checkboxes.length >= 1) {
                ...
                $.each(checkboxes, function (i, checkbox) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        dataType: "json",
                        data: jQuery.param({ checkedResourceId: checkedResourceId}) ,
                        cache: false,
                        success: success_get_resource,
                        error: function () {
                            alert("error");
                        }
                    });
                    ...
                    function success_get_resource(response) {

                        if (response !== false) {

                            displayDiv.append('<div class="resourceName" id="'+ response["resource_id"] +'" style="background-color:; color:;"><span class="resourceDetails" data-resourceid="' + response["resource_id"] + '" resource-details-bound="1">\n' +
                                '\n' + response["name"] + '</span> <input class="resourceId" type="hidden" name="additionalResources[]" value="' + response["resource_id"] + '"></div>');
                            var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user('+response['resource_id']+');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="'+response['user_id']+'">' + response['fname'] +' '+response['lname']+'</a><input type="hidden" class="id" name="invitationList[]" value="' + response['user_id'] + '" />' + '</div>';
    ...
    var InitializeAdditionalResources = function () {
        ...
        ajaxGet(url, null, function (data) {
            ...
            $.each(data, function (i, unavailableResourceId) {
                $('#physical_list option[class="'+unavailableResourceId+'"]').prop('disabled', true);
                $('#physical_list').selectpicker('render');
            });
    ...
    var InitializeAdditionalResourcesHuman = function () {
        ...
        ajaxGet(url, null, function (data) {
            $.each(data, function (i, unavailableResourceId) {
                $('#group_user_list_human option[class="'+unavailableResourceId+'"]').prop('disabled', true);
    ...
    var WireUpActions = function () {
        $('.create').click(function () {
            ...
            /* var primary = $('#primaryResourceContainer').find($("input[class='resourceId']")).length;
             var physical = $('.reservationResources').find($("input[class='resourceId']")).length;
             var physical_additional = $('#additionalResources').find($("input[class='resourceId']")).length;
             var human = $('#inviteeList input:hidden').length;
             var p = primary - 1;
             var t = p + physical_additional;*/
    ...
    participation.addInvitee = function (name, userId) {
        ...
        function success(response) {
            ...
            if ($('input[name=reservationAction]').val() == 'create') {
                if (response !== false) {
                    ...
                    /* displayDiv2.append('<div class="resourceName" id="' + response["resource_id"] + '" style="background-color:; color:;"><span class="resourceDetails" data-resourceid="' + response["resource_id"] + '" resource-details-bound="1">\n' +
                         '\n' + response["name"] + '</span> <input class="resourceId" type="hidden" name="additionalResources[]" value="' + response["resource_id"] + '"></div>');*/
                    var item = '<div class="user" id="'+response["resource_id"]+'">' + '<a href="#" class="remove" onclick="remove_user_add(' + response["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
                    elements.inviteeList.append(item);
                    $('.bindableUser').bindUserDetails();
                    participation.addedUsers.push(userId);
                    updateInviteeCount();
        ...
        if($('input[name=reservationAction]').val() == 'update') {
            ...
            function successed(responsed)
            {
                ...
                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user_add(' + responsed["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
...
function remove_user(id) {
    ...
    //$(".btnConfirmAddResourcesHuman").click();
    //AddResources();
}

function remove_user_add(id)
{
    $('#'+id).remove();
    $("#"+id).prop( "checked", false );
    //$(".btnConfirmAddResourcesHuman").click();
