...
interface IResourceService
{
    ...
    public function GetScheduleResources2($scheduleId, $includeInaccessibleResources, UserSession $user, $filter = null);
    /**
     * Gets resource list
     * @param bool $includeInaccessibleResources
     * @param UserSession $user
...
class ResourceService implements IResourceService
{
    ...
    public function GetScheduleResources2($scheduleId, $includeInaccessibleResources, UserSession $user, $filter = null)
    {
        ...
        $resources = $this->_resourceRepository->GetScheduleResources2($scheduleId);
        $resourceIds = $filter->FilterResources($resources, $this->_resourceRepository, $this->_attributeService);

        $arraySize = sizeof($resourceIds);

        if($resourceIds == null || sizeof($resourceIds) == $arraySize){
            $resources = $this->_resourceRepository->GetScheduleResources($scheduleId);
            $resourceIds = $filter->FilterResources($resources, $this->_resourceRepository, $this->_attributeService);
        }

        if(empty($filter->ResourceIds)) {
            $resources = $this->_resourceRepository->GetScheduleResources2($scheduleId);
            $resourceIds = $filter->FilterResources($resources, $this->_resourceRepository, $this->_attributeService);
        }
