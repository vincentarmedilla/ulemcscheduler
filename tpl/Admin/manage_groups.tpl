...
<script>
    $(document).ready(function(){
        $('#addGroupName').keyup(check_group_name);
    });

    function check_group_name() {
        var data = $('#addGroupName').val();
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response == 0){
                    $(".add_groups").removeAttr("disabled");
                    $("#message").html("").addClass("error-msg");
                } else {
                    $(".add_groups").attr("disabled", true);
                    $("#message").html("Group Name Already Exist").addClass("error-msg");
    ...
    <form id="addGroupForm" class="form-inline" role="form" method="post">
        <div class="panel panel-default" id="add-group-panel">
            ...
            <div class="panel-footer">
                {add_button class="btn-sm add_groups"}
    ...
    <table class="table" id="groupList">
        ...
        {foreach from=$groups item=group}
            ...
            <tr class="{$rowCss}" data-group-id="{$group->Id}" data-group-default="{$group->IsDefault}">
                ...
                <td><a href="#" class="update members" data-group-ids="{$group->Id}">{translate key='Manage'}</a></td>
...
<!--                         <input type="text" id="userSearch" class="form-control" size="40"/> -->
                        ...
                        <select id="userSearchGroups"  class="form-control" >
                            <option value="" selected>Select User</option>
                            {foreach from=$group_users key=myId item=i}
                                <option value="{$i.user_id}">
                                    {$i.fname} - {$i.email} {$smarty.get.gid}
                                </option>
                            {/foreach}
                        </select>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group has-feedback">
							...
							{else}
								<select id="groupAdmin" name="group_admin"   class="form-control">
    								<option value="" selected>Select Group</option>
        							{foreach from=$AdminGroup key=myId item=i}
        								{if $i.admin_group_id eq '1'}
